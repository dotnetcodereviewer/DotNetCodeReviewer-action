...
async function main() {
    ...
    console.log(`PR changed ${changed.length} file(s); ${csFiles.length} reviewable C# file(s): ${csFiles.map((f) => f.filename).join(", ") || "(none)"}`);

    if (csFiles.length === 0) {
        console.log("No changed C# files to review nothing to comment on. (The PR must add/modify a .cs file.)");
    ...
    console.log(`Reviewed ${csFiles.length} file(s); found ${allFindings.length} issue(s), ${inlineComments.length} on changed lines. Posting review...`);
    try {
        ...
        console.log("Posted PR review successfully.");
    } catch (e) {
        // A 403 here almost always means the workflow token can't write to pull requests.
        if (/\b403\b/.test(e.message)) {
            fail(
                "Could not post the PR review: the workflow token lacks write access to pull requests (HTTP 403).\n" +
                "Fix: (1) add `permissions: { pull-requests: write }` to your workflow, and (2) in the repo/org " +
                "Settings -> Actions -> Workflow permissions, choose 'Read and write permissions'. " +
                "Note: PRs opened from a fork cannot receive comments from the default token."
            );
        }
        // Otherwise (e.g. inline line-mapping rejected) fall back to a plain summary comment.
        ...
        try {
            ...
            console.log("Posted summary comment successfully.");
        } catch (e2) {
            if (/\b403\b/.test(e2.message)) {
                fail(
                    "Could not post a PR comment: the workflow token lacks write access (HTTP 403).\n" +
                    "Add `permissions: { pull-requests: write }` to your workflow and set 'Read and write permissions' " +
